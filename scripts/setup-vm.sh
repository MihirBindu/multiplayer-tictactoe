...
if [ -f "$APP_DIR/.env" ]; then
  ...
  echo " Press ENTER to auto-generate secure random values (recommended)"
  ...
  DEFAULT_NK_KEY=$(openssl rand -hex 16)

  read -r -p "POSTGRES_PASSWORD [auto: $DEFAULT_DB_PASS]: " INPUT_DB_PASS
  POSTGRES_PASSWORD="${INPUT_DB_PASS:-$DEFAULT_DB_PASS}"

  read -r -p "NAKAMA_SERVER_KEY [auto: $DEFAULT_NK_KEY]: " INPUT_NK_KEY
  NAKAMA_SERVER_KEY="${INPUT_NK_KEY:-$DEFAULT_NK_KEY}"
